 ...
 * Heatmap block class
 *
 * @copyright 2018 Hanna Haataja based on heatmap by  Michael de Raadt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_heatmap extends block_base {
    ...
    public function get_content() {
        ...
        // Get local settings
        $activitysince = $this->config->from;
        ...
        if (empty($views) || !isset($lastcached) || $lastcached < $now - $cachelife) {
            ...
            if ($useinternalreader) {
                ...
                switch ($activitysince) {
                    case 'oneDay':
                        $timesince = 'AND timecreated >= UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL  1 day))';
                        break;
                    case 'twoDays':
                        $timesince = 'AND timecreated >= UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 2 day))';
                        break;
                    case 'oneWeek':
                        $timesince = 'AND timecreated >= UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 7 day))';
                        break;
                    case 'twoWeeks':
                        $timesince = 'AND timecreated >= UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 14 day))';
                        break;
                    case 'month':
                        $timesince = 'AND timecreated >= UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 30 day))';
                        break;
                    case 'sinceforever':
                        $timesince = '';
                        break;
                    case 'sincestart':
                        $timesince = 'AND timecreated >= :coursestart';
                        break;
                }
                ...
                switch ($activitysince) {
                    case 'oneDay':
                        $timesince = 'AND l.time >= DATE_SUB(NOW(), INTERVAL  1 day)';
                        break;
                    case 'twoDays':
                        $timesince = 'AND l.time >= DATE_SUB(NOW(), INTERVAL 2 day)';
                        break;
                    case 'oneWeek':
                        $timesince = 'AND l.time >= DATE_SUB(NOW(), INTERVAL 7 day)';
                        break;
                    case 'twoWeeks':
                        $timesince = 'AND l.time >= DATE_SUB(NOW(), INTERVAL 14 day)';
                        break;
                    case 'month':
                        $timesince = 'AND l.time >= DATE_SUB(NOW(), INTERVAL 30 day)';
                        break;
                    case 'sinceforever':
                        $timesince = '';
                        break;
                    case 'sincestart':
                        $timesince = 'AND l.time >= :coursestart';
                        break;
                }
        ...
        switch($activitysince){
             case 'oneDay':
                        $this->content->text .= html_writer::div( get_string('oneDay', 'block_heatmap'),
                        'block_heatmap_sincecoursestart'
                        );
                        break;
                    case 'twoDays':
                        $this->content->text .= html_writer::div( get_string('twoDays', 'block_heatmap'),
                        'block_heatmap_sincecoursestart'
                        );
                        break;
                    case 'oneWeek':
                        $this->content->text .= html_writer::div( get_string('oneWeek', 'block_heatmap'),
                        'block_heatmap_sincecoursestart'
                        );
                        break;
                    case 'twoWeeks':
                        $this->content->text .= html_writer::div( get_string('twoWeeks', 'block_heatmap'),
                        'block_heatmap_sincecoursestart'
                        );
                        break;
                    case 'month':
                        $this->content->text .= html_writer::div( get_string('month', 'block_heatmap'),
                        'block_heatmap_sincecoursestart'
                        );
                        break;
                    case 'sinceforever':
                        $this->content->text .= html_writer::div( get_string('sinceforever', 'block_heatmap'),
                        'block_heatmap_sincecoursestart'
                        );
                        break;
                    case 'sincestart':
                       $this->content->text .= html_writer::div( get_string('sincecoursestart', 'block_heatmap'),
                        'block_heatmap_sincecoursestart'
                        );
                        break;
        }
